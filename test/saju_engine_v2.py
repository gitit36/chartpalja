...
def _check_yongshin_fit(stem: str, branch: str, yong_info: Dict[str, Any], day_stem: str) -> Dict[str, bool]:
    """대운 or 세운 간지가 용신/희신/기신에 해당하는지 판정"""
    # (참고) tmap 변수는 기존 코드에서 쓰는 경우가 있어 유지해둠
    _ = day_tengo_ohaeng(day_stem)
...
# ── 충격지수 계산 ──────────────────────────────
_REL_IMPACT = {"합": 0.9, "충": -1.5, "형": -1.0, "파": -1.2, "해": -0.7, "극": -0.6, "반합": 0.5}


def _impact_score(relations_list: Any) -> float:
    """관계 목록 → 충격지수 합산"""
    total = 0.0

    # relations_list 형태가 (a) dict-list, (b) str-list, (c) 단일 str 등 섞일 수 있어 방어적으로 처리
    for rel_group in relations_list or []:
        if isinstance(rel_group, dict):
            rels = rel_group.get("relations", []) or []
        elif isinstance(rel_group, str):
            rels = [rel_group]
        elif isinstance(rel_group, list):
            rels = rel_group
        else:
            rels = [str(rel_group)]

        for r in rels:
            for k, w in _REL_IMPACT.items():
                if k in r:
                    total += w
                    break
...
def _ohang_balance(stems_list: List[str], branches_list: List[str]) -> float:
    ...
    avg = total / 5.0
    variance = sum((v - avg) ** 2 for v in vals) / 5.0

    # 최대 분산(극단적으로 한 오행에 몰리는 경우)을 근사치로 잡아 정규화
    max_var = ((total - avg) ** 2) * 4 / 5 + (avg ** 2) / 5 if total > 0 else 1.0
    if max_var == 0:
        return 1.0

    return round(1.0 - (variance / max(max_var, 1.0)), 2)


# ── 종합운점수 산출 ────────────────────────────
def _composite_score(
    base: float,
    yong_fit: Dict[str, bool],
    unseong: str,
    gil_count: int,
    hyung_count: int,
    impact: float,
    ...
    """0~100 종합운점수"""
    sc = base
    if yong_fit.get("용신부합"):
        sc += 15
    if yong_fit.get("희신부합"):
        sc += 8
    if yong_fit.get("기신부합"):
        sc -= 12
...
def build_daewoon_detail(r: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for blk in blocks:
        ...
        # 신살
        gil, hyung = _check_incoming_shinsal(branch, day_stem, year_branch)

        # 충격지수
        impact = _impact_score(rels_w_orig)

        # 오행균형도 (원국 + 대운)
        all_stems = orig_stems + [stem]
        all_branches = orig_branches + [branch]
        balance = _ohang_balance(all_stems, all_branches)

        # 종합운점수
        composite = _composite_score(
            50, yfit, unseong, len(gil), len(hyung),
            impact, balance
        )
...
def build_yearly_timeline(
    ...
    for yi in range(span):
        ...
        # 세운 신살
        sw_gil, sw_hyung = _check_incoming_shinsal(sw_branch, day_stem, year_branch)

        # ── 보조지표 계산 ─────────────────────
        # 용신력: 대운+세운이 용신/희신 공급하는 정도 (-1 ~ +1)
        ypower = 0.0
        if dw["용신부합"]:
            ypower += 0.3
        if dw["희신부합"]:
        ...
        if sw_yfit["기신부합"]:
            ypower -= 0.25
        ypower = max(-1.0, min(1.0, round(ypower, 2)))

        # 충격지수
        sw_impact = _impact_score(sw_rels_orig)

        sw_dw_impact = 0.0
        for r_str in sw_rels_dw:
            for k, w in _REL_IMPACT.items():
                if k in r_str:
                    sw_dw_impact += w
                    break

        total_impact = round(dw["충격지수"] + sw_impact + sw_dw_impact, 2)

        # 오행균형도 (원국+대운+세운)
        all_s = orig_stems + [dw["stem"], sw_stem]
        all_b = orig_branches + [dw["branch"], sw_branch]
        balance = _ohang_balance(all_s, all_b)

        # 길신/흉살 카운트 (대운+세운)
        total_gil = len(dw["신살_길신"]) + len(sw_gil)
        total_hyung = len(dw["신살_흉살"]) + len(sw_hyung)
        ...
        sc += _UNSEONG_SCORE.get(sw_unseong, 0) * 0.5
        sc += len(sw_gil) * 2
        sc -= len(sw_hyung) * 2
        sc += sw_impact * 1.5
        sc += sw_dw_impact * 1.0
...
# ──────────────────────────────────────────────
# SECTION 20 : 차트 전용 출력 구조 (v3.2)
# ──────────────────────────────────────────────

def build_chart_payload(r: Dict[str, Any]) -> Dict[str, Any]:
    """
    enrich_saju 결과 → 프론트 차트 전용 JSON 구조 생성.
    """
    day_stem = r["원국"]["day"][0]
    yong = r["용신"]
    ...
    chart_data = {
        "meta": {
            ...
            "gender": r["입력"]["성별"],
            "dayStem": day_stem,
            "dayElement": STEM_ELEMENT[day_stem],
            "strength": r["신강신약"]["판정"],
            "geokguk": r["격국"]["격국"],
            "yongshin": {
                "label": yong.get("용신", ""),
        ...
        "보조지표_범례": {
            "용신력": {
                "description": "대운+세운이 용신/희신 오행을 공급하는 정도",
                "range": [-1.0, 1.0],
                "analogy": "RSI"
            },
            "충격지수": {
                "description": "충/형/파/해/극의 가중 합산",
                "range": [-5.0, 5.0],
                "analogy": "Volatility"
            },
            "길신카운트": {
                "description": "대운+세운 발현 길신 수",
                "range": [0, 20],
                "analogy": "Volume(양봉)"
            },
            "흉살카운트": {
                "description": "대운+세운 발현 흉살 수",
                "range": [0, 20],
                "analogy": "Volume(음봉)"
            },
            "오행균형도": {
                "description": "원국+대운+세운 합산 오행의 균형 (1=완전균형)",
                "range": [0.0, 1.0],
                "analogy": "MACD"
            },
        },
