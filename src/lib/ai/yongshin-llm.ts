...
2. 월령(月令)과 지장간을 분석하여 일간의 강약(극왕/태강/신강/중화신강/중화신약/신약/태약/극약)을 직접 판단하라.
3. 격국(格局)을 정하라.
4. 억부(抑扶)·조후(調候)·통관(通關)을 종합하여 용신을 정하라.
5. 신약~극약이면 일간을 돕는 오행(인성/비겁), 신강~극왕이면 설기·억제하는 오행(식상/재성/관살)이 용신.
6. 중화신강/중화신약이면 격국 주성을 보호하는 방향으로 용신을 정하라.
