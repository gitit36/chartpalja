...
export default function CompatJoinPage() {
  ...
  return (
    <MobileContainer>
      <div className="min-h-screen flex flex-col bg-gradient-to-b from-purple-50 to-white px-6 py-10">
        {loading ? (
          <div className="flex-1 flex items-center justify-center text-gray-400 text-sm">불러오는 중…</div>
        ) : error ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center">
            <div className="text-5xl mb-4">⏳</div>
            <h1 className="text-xl font-bold text-gray-900 mb-2">초대 링크를 사용할 수 없어요</h1>
            <p className="text-sm text-gray-500 mb-8">{error}</p>
            <Link href="/app/input" className="text-sm text-purple-600 font-semibold">내 차트 만들기 →</Link>
          </div>
        ) : meta ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center">
            <p className="text-sm text-purple-600 font-semibold mb-2">궁합 초대</p>
            ...
            <button
              type="button"
              onClick={goInput}
              className="w-full max-w-sm py-3.5 rounded-xl text-sm font-bold text-white bg-gradient-to-r from-purple-600 to-indigo-600 shadow-lg active:scale-[0.98] transition-all"
            >
              사주 입력하고 연결하기
            </button>
            <p className="text-[11px] text-gray-400 mt-4">유료 해설은 각자 생성해요</p>
          </div>
        ) : null}
      </div>
    </MobileContainer>
