...
export function BackButton({ fallback = '/', className = '' }: Props) {
  ...
  return (
    <button
      onClick={handleBack}
      className={`w-11 h-11 -ml-2.5 flex items-center justify-center text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-full text-lg transition-colors ${className}`}
      aria-label="뒤로가기"
    >
      &larr;
    </button>
