    ...
    <MobileContainer>
      <div className="min-h-screen flex flex-col">
        <header className="sticky top-0 z-10 bg-cp-bg border-b border-cp-border px-4 py-3 flex items-center gap-3">
          <BackButton />
          <h1 className="text-base font-bold text-cp-text">{title}</h1>
        </header>

        <main className="flex-1 px-5 pt-5 pb-10">
          {effectiveDate && (
            <p className="text-xs text-cp-muted mb-5">시행일자: {effectiveDate}</p>
          )}
          <div className="legal-content text-sm text-cp-text leading-relaxed space-y-4">
            {children}
          </div>
        </main>
