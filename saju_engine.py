...
def is_fwd(gender: str, year_stem: str) -> bool:
    """대운 순행 여부: 남양/여음 → 순행, 남음/여양 → 역행"""
    yang = YINYANG_STEM.get(year_stem, "陽") == "陽"
    male = gender.lower() in ("male", "남", "남자")
    return male == yang


def ipchun(year: int):
    """해당 연도 입춘(立春) datetime 반환 (315도)"""
    return _term_deg(year, 315)


def _year_gz(now) -> Tuple[int, str]:
    """현재 시점의 세운(연간지) 반환 – 입춘 기준"""
    y = now.year
    ip = ipchun(y)
    if now < ip:
        y -= 1
    idx = (ganzhi_index("甲子") + (y - 1984)) % 60
    return y, GANZHI_60[idx]


def _fms(year_stem: str) -> str:
    """연간(年干)으로 첫 월간(月干) 산출 – 오호연원법"""
    tbl = {"甲": "丙", "乙": "戊", "丙": "庚", "丁": "壬", "戊": "甲",
           "己": "丙", "庚": "戊", "辛": "庚", "壬": "壬", "癸": "甲"}
    return tbl[year_stem]
